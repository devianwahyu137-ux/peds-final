...
export function SectorCard({ sector }) {
  ...
  return (
    <div className="card-tier-3 !p-0 overflow-hidden transition-all duration-200 border border-slate-200 dark:border-neutral-800">
      {/* ── CARD HEADER — always visible ── */}
      <button
        onClick={toggle}
        className="w-full text-left p-4 flex flex-col md:flex-row md:items-center justify-between cursor-pointer hover:bg-slate-100 dark:hover:bg-neutral-900/40 transition-colors gap-4"
      >
        <div className="flex items-center gap-3">
          {/* Sector icon */}
          <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-neutral-800 flex items-center justify-center text-lg">
            {sector.icon}
          </div>
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="font-bold text-slate-800 dark:text-neutral-200">{sector.sector}</span>
              <span
            ...
            <div className="flex items-center gap-3 mt-1 text-[10px] text-slate-500 dark:text-neutral-500 uppercase tracking-widest font-mono">
              <span>Target Alokasi: <span className="font-bold text-slate-700 dark:text-neutral-300">{sector.targetPct}%</span></span>
              <span>•</span>
              <span style={{ color: riskConf.color }}>{riskConf.label}</span>
            </div>
          </div>
        </div>

        {/* Ticker count + expand chevron */}
        <div className="flex items-center gap-4 ml-13 md:ml-0 text-xs font-mono text-slate-400 dark:text-neutral-500">
