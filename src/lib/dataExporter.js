...
export function exportPortfolioCSV({ scenarioId, weights, analytics, macroInputs }) {
  ...
  const rows = [
    ...
    ['USD/IDR',       `Rp ${formatIDR(macroInputs?.usdIdr ?? 17700)}`,  (useRootStore.getState().liveData?.usdIdr?.v != null && useRootStore.getState().liveData?.usdIdr?.src === 'supabase_live') ? 'LIVE (delay ~5 mnt)' : 'Estimasi (per Juni 2026)'],
